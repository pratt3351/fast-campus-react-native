...
import {onClickFavorite} from '../actions/photos';
...
export const ImageDetailScreen = ()=>{
    ...
    const isFavorite = useSelector((state)=> {
        return state.favorite.favorite.filter((item)=> item === route.params.url).length > 0;
    })
    ...
    const onPressHeart = useCallback(()=>{
        console.log('onPressHeart', route.params.url)
    ...
    return (
        <View style={{flex:1, borderColor:'red'}}>
            ...
            <Header>
                ...
                <Header.Icon iconName={isFavorite ? 'heart' : 'heart-outline'} onPress={onPressHeart}></Header.Icon>
            </Header>
